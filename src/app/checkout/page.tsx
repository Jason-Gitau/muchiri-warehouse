...
export default function CheckoutPage() {
  ...
  const checkAuth = async () => {
    try {
      ...
      setDistributorId(distributorData?.id || null);
      setLoading(false);
    } catch (error) {
  ...
  const handleCheckout = async () => {
    ...
    if (!distributorId) {
      alert('Distributor information not found');
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        clearCart();
        alert(`Order ${data.orderNumber} created successfully! Total: KSh ${data.totalAmount.toLocaleString()}`);
      ...
      {cart.length === 0 ? (
        <div className="bg-white rounded-lg shadow p-12 text-center">
          ...
          <p className="text-gray-600 mb-6">
            Add some products from the warehouse to get started
          </p>
          <button
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="text-sm text-blue-800">
                      ...
                      <p>
                        Order will be created with payment status UNPAID. Payment can be
